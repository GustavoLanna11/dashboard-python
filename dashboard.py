...
# Criação de um menu para navegação entre cidades
menu = st.sidebar.selectbox("Selecione uma região", ["São Paulo", "Rio de Janeiro"])

if menu == "São Paulo":
    st.header("Bem-vindo à Dashboard de São Paulo!")
    ...
    df = pd.read_csv("inventario_maquinas_exemplo2.csv", sep=";", encoding="ISO-8859-1")
    df = pd.read_csv("inventario_maquinas_exemplo2.csv", sep=";", encoding="latin1")
